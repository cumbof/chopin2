...
class HDModel(object):
    ...
    def __init__(
        ...
        trainData: list
            Training dataset.
        trainLabels: list
            Class labels of the training observations.
        testData: list
            Test dataset.
        testLabels: list
            Class labels of the test observations.
        D: int
        ...
        levelsdir: str
            Path to the folder in which the HD levels are located.
        k_fold: int
            Slice n-th for cross validation.
        spark: bool
            Enable the construction of the HD vectors on a Apache Spark distributed environment.
        slices: int
            Number of slices for distributing vectors over the Apache Spark cluster.
        master: str
            Address to the Apache Spark master node.
        memory: str
            Maximium memory allocated by Apache Spark.
        gpu: bool
            Enable the construction of the HD vectors on NVidia GPUs.
        tblock: int
            Number of blocks on NVidia GPUs.
        nproc: int
            Number of processors for running the construction of the hypervectors in multiprocessing
        verbose: bool
            Print messages.
        log: str
            Path to the log file.
        seed: int
            Seed for reproducing the same random hypervectors.

        Raises
        ------
        Exception
            If the list with training data and the list with training labels have different sizes;
            If the list with test data and the list with test labels have different sizes
        """

        if len(trainData) != len(trainLabels):
            raise Exception("Training data and training labels do not have the same size!")

        if len(testData) != len(testLabels):
            raise Exception("Testing data and testing labels do not have the same size!")
    ...
    def buildBufferHVs(self, mode, verbose=False, log=None):
        ...
        ----------
        mode: str
            Two modes allowed: "train" and "test".
        verbose: bool
            Print messages.
        log: str
            Path to the log file.
        ...
        if mode == "train":
            train_bufferHVs = os.path.join(
                self.workdir,
                "train_bufferHVs_{}_{}_{}_{}{}".format(
                    self.D,
                    self.totalLevel,
                    self.hashId,
                    self.k_fold,
            ...
            test_bufferHVs = os.path.join(
                self.workdir,
                "test_bufferHVs_{}_{}_{}_{}{}".format(
                    self.D, 
                    self.totalLevel, 
                    self.hashId, 
                    self.k_fold,
...
def oneHvPerClass(inputLabels, inputHVs, gpu=False, tblock=32):
    ...
    inputHVs: list
        HD encoded input data.
    gpu: bool
        Enable the execution on NVidia GPUs.
    tblock: int
        Number of blocks on NVidia GPUs.

    Returns
    -------
    It returns the class hypervectors.
    """
...
def numToKey(value, levelList):
    ...
    value: float
        Feature value.
    levelList: list
        List of level hypervector ranges.

    Returns
    -------
    The index of the lever hypervector corresponding to a specific value.
    """
...
def getlevelList(buffers, totalLevel):
    ...
    buffers: list
        Data matrix.
    totalLevel: int
        Number of level hypervector ranges.
...
def genLevelHVs(totalLevel, D, gpu=False, tblock=32, verbose=False, log=None, seed=0):
    ...
    totalLevel: int
        Number of level hypervector.
    D: int
        Dimensionality of the hypervectors.
    gpu: bool
        Enable the generation of the HD levels on NVidia GPUs.
    tblock: int
        Number of blocks on NVidia GPUs.
    verbose: bool
        Print messages.
...
def EncodeToHV_wrapper(inputBuffer, position, D=10000, levelHVs=dict(), levelList=list()):
    ...
    ----------
    inputBuffer: list
        Input data that must be encoded.
    position: int
        Position of chunk data for multiprocessing.
    D: int
        Dimensionality of the hypervectors.
    levelHVs: dict
        Level hypervectors.
    ...
    Returns
    -------
    A tuple with the vector position and the function to encode the vector.
    """
...
def checkVector(classHVs, inputHV, labelHV):
    ...
    inputHV: numpy.array
        Input hypervector.
    labelHV: str
        Label of the input hypervector.

    Returns
    -------
    A tuple with a 1 in case the vector has been correctly predicted, 0 otherwise,
...
def trainOneTime(classHVs, trainHVs, trainLabels, verbose=False, log=None):
    ...
    trainHVs: list
        Train hypervectors.
    trainLabels: list
        Label of the train hypervectors.
    verbose: bool
        Print messages.
    log: str
        Path to the log file.
    ...
    Returns
    -------
    The retrained class vectors and the model's error rate.
    """
...
def test(
    ...
    testHVs: list
        Test hypervectors.
    testLabels: list
        Label of the test hypervectors.
    spark: bool
        Enable the construction of the HD vectors on a Apache Spark distributed environment.
    slices: int
        Number of slices for distributing vectors over the Apache Spark cluster.
...
def trainNTimes(
    ...
    stop: bool
        Stop retraining if the error rate doesn't change with respect to the previous retraining iteration.
    spark: bool
        Enable the construction of the HD vectors on a Apache Spark distributed environment.
    slices: int
        Number of slices for distributing vectors over the Apache Spark cluster.
    ...
    Returns
    -------
    The accuracy score for each of the retraining iterations.
    """
...
def buildHDModel(
    ...
    """Create an HD model object, encode the training and test datasets, 
    and perform the initial training of the HD model.
...
def buildDatasetPKL(filepath, separator=","):
    ...
    separator: str
        Separator used to split fields in the input dataset.
    training: float
        Percentage of observations used to build the training (and implicitly the test) dataset.
    seed: int
        Seed for reproducing the same training and test datasets.

    Returns
    -------
    A tuple with the list of features, the list with the actual numerical dataset,
...
def percentage_split(data, classes, training=80.0, seed=0):
    ...
    data: list
        List with data.
    classes: list
        List of labels for each observation in data.
    training: float
        Percentage of observations used to build the training (and implicitly the test) dataset.
    seed: int
        Seed for reproducing the same training and test datasets.
    ...
    Returns
    -------
    A tuple with the training data, training labels, test data, and test labels.
    """
...
def buildDatasetFLAT(data, labels, features, outpath, sep=","):
    ...
    data: list
        Training dataset.
    labels: list
        Class labels of the training dataset.
    features: list
        Feature IDs.
    outpath: str
        Path to the output dataset file.
    sep: str
        Separator used to split fields in the output dataset.
    """
...
def printlog(message, data=list(), print_threshold=100, end_msg=None, verbose=False, out=None):
    ...
    message: str
        Custom message.
    data: list
        List of data to be dumped to stdout/file.
    print_threshold: int
        Do not print data if its length is greater than this threshold.
    end_msg: str
        Print this message at the end.
    verbose: bool
...
def cleanup(group_dir, levels_dir, dimensionality, levels, features_hash, k_folds=1, spark=False, skip_levels=False):
    ...
    levels_dir: str
        Path to the folder with the HD levels.
    dimensionality: int
        Dimensionality of the hypervectors.
    levels: int
        Number of HD levels.
    ...
    k_fold: int
        Slice n-th for cross validation.
    spark: bool
        Did you use Apache Spark?
    skip_levels: bool
        Do not remove the HD levels.
    """
