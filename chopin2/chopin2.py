...
def number(typev, minv=None, maxv=None):
    """Define custom numeric intervals.
    """
...
def chopin2():
    ...
    if features and allData and allLabels:
        ...
        if args.features:
            use_features = [line.strip() for line in open(args.features).readlines() if line.strip()]
        ...
        if args.select_features:
            ...
            if args.group_min > max_group:
                raise Exception(
                    ("An error has occurred while defining groups. "
            ...
            if not os.path.exists(summary_filepath):
                with open(summary_filepath, "a+") as summary:
                    header = (
                        ...
                        "# Dimensionality: {}\n"
                        "# Number of levels: {}\n"
                        "# Max retraining iterations: {}\n"
                        "# Accuracy threshold (stop condition): {}\n"
                        ...
                        args.accuracy_uncertainty_perc
                    )
                    ...
                    for comb_features in itertools.combinations(best_features, group_size):
                        ...
                        fun.printlog(
                            (
                                ...
                                int(combinations_counter), int(combinations),
                                group_size
                            ),
                        ...
                        # Check which features are in the currect set of features
                        ...
                        for model in models:
                            accuracy, retraining = fun.trainNTimes(
                                model.classHVs, 
                                model.trainHVs, 
                                model.trainLabels, 
                                model.testHVs, 
                        ...
                        fun.printlog(
                            (
                                "Total elapsed time (testing) {}s\n"
                                "The maximum reached accuracy is {} after {} retrainings\n"
                                "Stopped after {} retraining iterations\n"
                            ).format(
                                int(t1acc-t0acc),
                                avg_accuracy, avg_retraining,
                                len(accuracy)-1
                            ),
                            verbose=args.verbose, out=run)

                        # Keep track of the best accuracy for the current group size
                        if group_size in mapping:
                            # Get the best accuracy in group
                            if avg_accuracy >= mapping[ group_size ][0]["accuracy"]:
                                threshold = avg_accuracy-(avg_accuracy*args.accuracy_uncertainty_perc)/100.0

                            elif avg_accuracy < mapping[group_size][0]["accuracy"]:
                                threshold = mapping[group_size][0]["accuracy"]-(mapping[group_size][0]["accuracy"]*args.accuracy_uncertainty_perc)/100.0
                            ...
                            mapping[group_size] = sorted(
                                [run for run in mapping[group_size] if run["accuracy"] >= threshold], 
